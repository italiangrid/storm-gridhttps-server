...
public class CopyMethodAuthorization extends AbstractMethodAuthorization {
	...
	public CopyMethodAuthorization(StorageArea srcSA, StorageArea destSA) {
		this.srcSA = srcSA;
		this.destSA = destSA;
	}
	...
	public AuthorizationStatus isUserAuthorized() {
		...
		if (httpHelper.hasDestinationHeader()) { 
			...
			if (askAuth(Constants.CP_FROM_OPERATION, srcPath)) {
				String destPath = destSA.getRealPath(destURI.getPath());
				String operation = httpHelper.isOverwriteRequest() ? Constants.CP_TO_OVERWRITE_OPERATION : Constants.CP_TO_OPERATION;
