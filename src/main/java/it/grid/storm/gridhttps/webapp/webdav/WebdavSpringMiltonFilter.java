...
public class WebdavSpringMiltonFilter implements javax.servlet.Filter {
	...
	private static final Logger log = LoggerFactory
		.getLogger(WebdavSpringMiltonFilter.class);
	...
	/**
	 * Resources with this as the first part of their path will not be served from
	 * milton. Instead, this filter will allow filter processing to continue so
	 * they will be served by JSP or a servlet
	 */
	...
	public void init(FilterConfig fc) throws ServletException {
		...
		try {
			context = new ClassPathXmlApplicationContext(
				new String[] { filterConfig.getInitParameter("contextConfigLocation") });
		...
		String mappingContextPath = File.separator
			+ Configuration.getGridhttpsInfo().getMapperServlet().getContextPath()
			+ File.separator
			+ Configuration.getGridhttpsInfo().getMapperServlet().getContextSpec();
		String fileTransferContextPath = File.separator
			+ Configuration.getGridhttpsInfo().getFiletransferContextPath();
		String[] davExcluded = { "/index.jsp", fileTransferContextPath,
			mappingContextPath };
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain fc)
		...
		if (req instanceof HttpServletRequest) {
			...
			// Allow certain paths to be excluded from milton, these might be other
