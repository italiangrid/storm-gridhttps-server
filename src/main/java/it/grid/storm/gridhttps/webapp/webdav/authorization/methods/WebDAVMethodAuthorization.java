...
public abstract class WebDAVMethodAuthorization extends AbstractMethodAuthorization {
	...
	public static enum Permission { READ, READWRITE };
	...
	private AuthorizationStatus isAnonymousAuthorized(StorageArea sa,
		...
		/* Knowing that:
		 * - Permissions requested can be READ or READWRITE
		 * - A storage area can be HTTPReadable or HTTPReadable + HTTPWritable 
		 * The user is not AUTHORIZED to access if:
		 * - the storage area is not HTTPReadable
		 * - the storage area is not HTTPWritable and the permission asked is READWRITE
		 * */
