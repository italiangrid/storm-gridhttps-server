...
public class StorageAreaManager {
	...
	public static void init(List<StorageArea> SAList) {
		if (storageAreas == null) {
			log.info("Initializing the Storage Area manager with following StorageAreas: "
					+ SAList.toString());
			storageAreas = SAList;
		}
	}

	public static void initFromStormBackend(String hostname, int port)
			throws Exception {
		String stormBackendIP = InetAddress.getByName(hostname)
				.getHostAddress();
		ConfigurationParameters stormBackendParameters = new ConfigurationParameters(
				hostname, stormBackendIP, port);
	 ...
	 */
	private static LinkedList<StorageArea> populateStorageAreaConfiguration(
			final ConfigurationParameters stormBackendParameters)
			throws Exception {
		...
		try {
			...
			throw new Exception(
					"Error contacting Configuration Discovery service.");
		} catch (IOException e) {
		...
		if (entity != null) {
			...
			try {
				...
				log.error("Unable to get the input content stream from server answer. IllegalStateException "
						+ e.getLocalizedMessage());
				throw new Exception(
						"Error comunicationg with the Configuration Discovery service.");
			} catch (IOException e) {
			...
			throw new Exception(
					"Unable to get a valid configuration discovery response from the server.");
		}
		...
		/* DA RIMUOVERE!!! */
		//String output = "name=DTEAMT0D1-FS&root=/storage/dteamt0d1&stfnRoot=/dteamt0d1;/dteam&protocols=https:name=ATLAST0D1-FS&root=/storage/atlast0d1&stfnRoot=/atlast0d1&protocols=https;http";
	 ...
	 * @throws ServletException
	 */
	private static URI buildConfigDiscoveryServiceUri(
			final ConfigurationParameters stormBackendParameters)
			throws Exception {
		log.debug("Building configurationd discovery rest service URI");
		String path = "/" + ConfigDiscoveryServiceConstants.RESOURCE + "/"
				+ ConfigDiscoveryServiceConstants.VERSION + "/"
				+ ConfigDiscoveryServiceConstants.LIST_ALL_KEY;
		URI uri;
		try {
			uri = new URI("http", null,
					stormBackendParameters.getStormBackendHostname(),
					stormBackendParameters.getStormBackendRestPort(), path,
					null, null);
		} catch (URISyntaxException e) {
			log.error("Unable to create Configuration Discovery URI. URISyntaxException "
					+ e.getLocalizedMessage());
	...
	private static List<StorageArea> decodeStorageArea(String sAEncoded) {
		if (sAEncoded == null) {
			...
			throw new IllegalArgumentException(
					"Received a null encoded storage area");
		}
		...
		String[] SAFields = sAEncoded.trim().split(
				"" + ConfigDiscoveryServiceConstants.VFS_FIELD_SEPARATOR);
		for (String SAField : SAFields) {
			String[] keyValue = SAField.trim().split(
					"" + ConfigDiscoveryServiceConstants.VFS_FIELD_MATCHER);
			if (ConfigDiscoveryServiceConstants.VFS_NAME_KEY
					.equals(keyValue[0])) {
				name = keyValue[1];
				log.debug("Found name: " + name);
				continue;
			}
			if (ConfigDiscoveryServiceConstants.VFS_ROOT_KEY
					.equals(keyValue[0])) {
				root = keyValue[1];
				log.debug("Found File System Root: " + root);
				continue;
			}
			if (ConfigDiscoveryServiceConstants.VFS_STFN_ROOT_KEY
					.equals(keyValue[0])) {
				String[] stfnRootArray = keyValue[1]
						.trim()
						.split(""
								+ ConfigDiscoveryServiceConstants.VFS_STFN_ROOT_SEPARATOR);
			...
			if (ConfigDiscoveryServiceConstants.VFS_ENABLED_PROTOCOLS_KEY
					.equals(keyValue[0])) {
				String[] protocolsArray = keyValue[1]
						.trim()
						.split(""
								+ ConfigDiscoveryServiceConstants.VFS_ENABLED_PROTOCOLS_SEPARATOR);
		...
		if (name == null || root == null || stfnRootList.size() == 0) {
			log.warn("Unable to decode the storage area. Some fileds are missin: name="
					+ name
					+ " FSRoot="
					+ root
					+ " stfnRootList="
		...
		for (String stfnRoot : stfnRootList) {
			log.debug("Decoded storage area: [" + name + "," + root + ","
					+ stfnRoot + ", " + StorageArea.protocolToStr(protocol) + "]");
	 ...
	 */
	public static StorageArea getMatchingSA(String localFilePath)
			throws IllegalArgumentException, IllegalStateException {
		if (localFilePath == null) {
			log.error("Unable to match StorageArea, the provided localFilePath is null");
			throw new IllegalArgumentException(
					"Provided localFilePath il null!");
		}
		if (!initialized()) {
			log.error("Unable to match StorageArea, class not initialized. "
					+ "Call init() first");
			throw new IllegalStateException(
					"Unable to match any StorageArea, class not initialized.");
		}
		...
		for (StorageArea storageArea : storageAreas) {
			if (localFilePath.startsWith(storageArea.getFSRoot())
					&& (mappedSA == null || storageArea.getFSRoot().length() > mappedSA
							.getFSRoot().length())) {
